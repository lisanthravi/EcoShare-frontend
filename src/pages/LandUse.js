...
const LandUse = () => {
    ...
    const fetchTips = async () => {
        try {
            const response = await axios.get('http://localhost:5000/api/land-use');
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:5000/api/land-use', formData);
    ...
    return (
        <div className='ser-outer'>
            <br></br>
            <br></br>
            <h1>Land Use Tips and Projects</h1>
            <br></br>
